...
begin        
    ...
    if((Pais is not null) and (Estado is not null) and (Edad is not null)) then
       ...
       for select foto_persona, pasaporte_persona, nombre1_persona, COALESCE(nombre2_persona,' ')as nombre2_persona, apellido1_persona, apellido2_persona, 
                    edad_a(fechanac_persona) as age, genero_persona, l.identificacion_lugar.nombre,
                    ...
                    and Edad = edad_a(fechanac_persona)
...
begin
    ...
    for select p.pasaporte_persona"N° Pasaporte", p.foto_persona"Foto", p.nombre1_persona"Primer nombre", COALESCE(p.nombre2_persona,' ')"Segundo nombre",
            p.apellido1_persona"Primer Apellido", p.apellido2_persona"Segundo Apellido", edad_a(p.fechanac_persona)"Edad", l3.identificacion_lugar.imagen"Pais Residencia",
...
foto_pais blob;
estado_name varchar2(100);
begin
if (estado is null) then
--por pais, muestra todos los estados de un pais (correguir)
-- l.identificacion_lugar.imagen "Foto"
    delete from temporal_reporte4;

    select min(l.id_lugar) into min_ciudad from lugar l, lugar l2, lugar l3 where l.id_lugar_lugar = l2.id_lugar and l2.id_lugar_lugar = l3.id_lugar 
        and l3.identificacion_lugar.nombre = pais;
    select max(l.id_lugar) into max_ciudad from lugar l, lugar l2, lugar l3 where l.id_lugar_lugar = l2.id_lugar and l2.id_lugar_lugar = l3.id_lugar 
        and l3.identificacion_lugar.nombre = pais;
    ...
    loop
        ...
        select count(*) into cant_infectados from persona where  id_lugar_persona = ciudad
        and status_persona = 'Infectado';
        --cantidad de fallecidos
        select count(*) into cant_fallecidos from persona where  id_lugar_persona = ciudad
        and fechadef_persona is not null;
        --cantidad de recuperados
        select count(*) into cant_recuperados from persona where  id_lugar_persona = ciudad
        and status_persona = 'Sano';
        
        select l.identificacion_lugar.imagen "Foto" into foto_pais from lugar l  where l.identificacion_lugar.nombre = pais;
        select l2.identificacion_lugar.nombre "Estado" into estado_name from lugar l, lugar l2, lugar l3 where l.id_lugar = ciudad and l.id_lugar_lugar = l2.id_lugar and l2.id_lugar_lugar = l3.id_lugar and l3.identificacion_lugar.nombre = pais;
        
        dbms_output.put_line('Estado: '||estado_name);
        
        INSERT INTO temporal_reporte4 VALUES (
            foto_pais,
            estado_name,
            poblacion,
            cant_infectados,
            ((cant_infectados*100)/poblacion),
            cant_recuperados,
            ((cant_recuperados*100)/poblacion),
            cant_fallecidos,
            ((cant_fallecidos*100)/poblacion)
        );
...
--por estado (pasas el estado y devuelves un solo valor)
    ...
    for select l3.identificacion_lugar.imagen "Foto", l2.identificacion_lugar.nombre"Estado",
            ...
            (select count(*) from persona where  id_lugar_persona = (select l.id_lugar from lugar l, lugar l2 where l.id_lugar_lugar = l2.id_lugar 
                and l2.identificacion_lugar.nombre = estado) and status_persona = 'Sano')"Cantidad Recuperados",
            (select count(*) from persona where  id_lugar_persona = (select l.id_lugar from lugar l, lugar l2 where l.id_lugar_lugar = l2.id_lugar 
                and l2.identificacion_lugar.nombre = estado) and status_persona = 'Sano')/(select l2.poblacion_lugar from lugar l, lugar l2 
...
--ambos parametros (devuelve un solo valor) 
    ...
    for select l3.identificacion_lugar.imagen "Foto", l2.identificacion_lugar.nombre"Estado",
            ...
            (select count(*) from persona where  id_lugar_persona =
                (select l.id_lugar from lugar l, lugar l2 where l.id_lugar_lugar = l2.id_lugar and l2.identificacion_lugar.nombre = estado)
             and status_persona = 'Sano')"Cantidad Recuperados",
            round((select count(*) from persona where  id_lugar_persona =
                (select l.id_lugar from lugar l, lugar l2 where l.id_lugar_lugar = l2.id_lugar and l2.identificacion_lugar.nombre = estado)
             and status_persona = 'Sano')/(select l.poblacion_lugar from lugar l 
...
begin
    ...
    if ( modelo is null and pais is not null) then 
        ...
        select l.identificacion_lugar.imagen"Pais", to_char(la.fechai_la, 'DD/MM/YYYY')"Fecha de Inicio", 
            COALESCE(to_char(la.fechaf_la, 'DD/MM/YYYY'),'No posee aún')"Fecha de fin", a.nombre_aislamiento"Modelo aplicado"
    ...
    -- Sin pais y sin modelo
        ...
        OPEN rep5_cursor FOR 
        select l.identificacion_lugar.imagen"Pais", to_char(la.fechai_la, 'DD/MM/YYYY')"Fecha de Inicio", COALESCE(to_char(la.fechaf_la, 'DD/MM/YYYY'),'No posee aún')"Fecha de fin", 
...
/
create or replace function Infections2(fechai varchar2, fechaf varchar2,pais varchar2) return fi2
is
    fechaIn fi2:= fi2();
   -- Qpob number;
   -- nombreP varchar2(30);
...
begin
    if(pais is not null) then
        ...
        dbms_output.put_line('Dias: '||dias);
        fechaIn.extend(dias);
        fechaD:= to_date(fechai,'DD-MM-YYYY');
    
        for i in 1..dias loop
            dbms_output.put_line('FechaD: '||fechaD);
            ...
            and TO_CHAR(hm.feciinicialingreso_histm,'DD-MON-YYYY') = fechaD;
...
--        select round(fechaf - fechai) into dias from dual;
        select round(to_date(fechaf,'DD-MM-YYYY') - to_date(fechai,'DD-MM-YYYY'),0) into dias from dual;
        fechaIn.extend(dias);
        fechaD:= to_date(fechai,'DD-MM-YYYY');
...
create or replace procedure Grafico(p_cursor OUT sys_refcursor,Pais Varchar2, fechai varchar2, fechaf varchar2)
is    
begin  
    if(Pais is not null) then
       Open p_cursor 
